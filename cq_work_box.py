...
screw_post_rect = (
    width - (screw_post_diam / 2), 
    length - (screw_post_diam / 2)
)
...
base = base.faces("<Z").workplane(centerOption="CenterOfMass", offset=-shell_thickness)\
    ...
    .vertices().circle(screw_post_diam / 2).extrude(-height)\
    .rect(screw_post_rect[0], screw_post_rect[1], forConstruction=True)\
    .vertices().circle(screw_hole_diam / 2).cutBlind(-height)\
...
lid = base.faces(">Z").workplane(centerOption="CenterOfMass")\
    .box(lid_width, lid_length, 4.5, combine=False)\
    ...
    .faces("<Z").workplane()\
    .rect(screw_post_rect[0], screw_post_rect[1], forConstruction=True)\
    .vertices().circle(screw_post_diam / 2).cutBlind(-shell_thickness / 2)\
    .faces(">Z").workplane()\
    .rect(screw_post_rect[0], screw_post_rect[1], forConstruction=True)\
    .vertices().cboreHole(screw_hole_diam, 6.5, 3.1) # M3 socket head
